...
export default function Stream({ drama, stream, currentEpisode, id }: Props) {
    const data = stream.data_stream[0] || {};
    const [quality, setQuality] = useState<"360p" | "480p" | "720p">(
        data["720p"] ? "720p" : data["480p"] ? "480p" : "360p"
    );

    const videoUrl = data[quality];
    const pageTitle = currentEpisode 
        ? `${drama.title} - ${currentEpisode.episode_label}` 
        : `Menonton ${drama.title}`;
        ...
        <div className="bg-slate-900 text-slate-200 min-h-screen font-sans pb-24 selection:bg-blue-500/30">
            ...
            <div className="max-w-7xl mx-auto p-6 md:p-12">
                ...
                <nav className="flex items-center gap-3 text-xs font-black uppercase tracking-tighter mt-12 mb-8">
                    <Link href={route('drama.index')} className="text-slate-500 hover:text-blue-500 transition-colors">HOME</Link>
                    <span className="text-slate-700">/</span>
                    <Link href={route('drama.show', { id: drama.id })} className="text-slate-500 hover:text-blue-500 transition-colors truncate max-w-[150px]">{drama.title.split(' ').slice(0,3).join(' ')}...</Link>
                    <span className="text-slate-700">/</span>
                    <span className="text-blue-400">{currentEpisode?.episode_label?.toUpperCase() || `EPISODE ${id}`}</span>
                </nav>
                        ...
                        <div className="bg-slate-800/30 p-8 rounded-[2.5rem] border border-slate-700/50 backdrop-blur-3xl flex flex-col md:flex-row items-center justify-between gap-8">
                            <div className="space-y-2 flex-1">
                                ...
                                <p className="text-blue-400 text-sm font-black tracking-widest uppercase flex items-center gap-2">
                                    ...
                                    NOW PLAYING: {currentEpisode?.episode_label || `Episode ${id}`}
                                </p>
                            </div>
                                    ...
                                    data[q] && (
                                        ...
                                        >
                                            ...
                                            <span className="block text-[8px] opacity-60 font-medium mt-0.5">
                                                {data[`${q}_size` as keyof StreamData] || 'Unknown MB'}
                                            </span>
                                        </button>
                                    )
                                ))}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-4 overflow-x-auto pb-4 px-4 scrollbar-hide">
                                {drama.data_episode?.map((ep) => (
                                    <Link 
                                        key={ep.episode_id}
                                        href={route('drama.watch', { drama_id: drama.id, streaming_id: ep.streaming })}
